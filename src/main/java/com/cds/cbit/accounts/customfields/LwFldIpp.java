package com.cds.cbit.accounts.customfields;

import com.portal.pcm.StrField;

/**
 * Specific Field subclasses. This subclasses of <code>Field</code> is used with the FList class to
 * specifiy which field is being accessed, and its type. The type information is used to provide
 * compile time type checking. These classes are auto generated.
 * 
 * @version 1.0 Thu Mar 22 17:09:35 IST 2018
 * @author Autogenerated
 */

public class LwFldIpp extends StrField {

  private static final long serialVersionUID = 1L;

  /**
   * Constructs an instance of <code>LwFldIpp</code>.
   */
  public LwFldIpp() {
    super(20019, 5);
  }

  /**
   * Returns an instance of <code>LwFldIpp</code>.
   * 
   * @return An instance of <code>LwFldIpp</code>.
   */
  public static synchronized LwFldIpp getInst() {
    if (me == null) {
      me = new LwFldIpp();
    }
    return me;
  }

  private static LwFldIpp me;
}
